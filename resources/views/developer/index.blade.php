...
<div class="row g-4">
    ...
    <div class="col-12 mb-2 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
        <div>
            <h2 class="h3 font-outfit text-white mb-1">Developer Tools & API Sandbox</h2>
    ...
    <div class="col-12 col-lg-7">
        ...
        <!-- Access Tokens Card -->
        <div class="overflow-hidden border border-slate-800 rounded-4 shadow-lg mb-4 p-4" style="background: linear-gradient(135deg, #7c3aed, #4f46e5);">
            <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3 mb-4">
                <div>
                    <h4 class="m-0 text-white font-outfit font-bold">Access Tokens</h4>
                ...
                @else
                    <div class="table-responsive">
                        <table class="table align-middle m-0" style="--bs-table-bg: transparent; --bs-table-border-color: #f1f5f9; color: #1e293b !important;">
                            <thead class="text-slate-400" style="font-size: 11px;">
                                <tr>
                                    ...
                                    <tr>
                                        <td class="ps-0 border-0">
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <button type="button" class="btn p-0 text-slate-400 hover:text-slate-600 border-0 bg-transparent shadow-none" onclick="toggleTokenVisibility({{ $key->id }})" title="Toggle Visibility">
                                                ...
                                                <!-- Copy Button -->
                                                <button type="button" class="btn p-0 text-slate-400 hover:text-slate-600 border-0 bg-transparent shadow-none" onclick="copyToken({{ $key->id }}, '{{ $key->key_prefix }}')" title="Copy to clipboard">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16">
                                        ...
                                        <td class="text-end pe-0 border-0">
                                            ...
                                            <form action="{{ route('developer.keys.destroy', $key->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this API Key? This will immediately revoke access.')" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn p-0 text-slate-400 hover:text-red-600 bg-transparent border-0 shadow-none" title="Revoke Token">
        ...
        <div class="d-flex border-bottom border-slate-800 mb-4 pb-0" id="sandboxTabs" role="tablist" style="margin-top: 1.5rem;">
            ...
            <div class="tab-pane fade show active" id="runner" role="tabpanel" aria-labelledby="runner-tab">
                <div class="card glass-card p-4">
                    <div class="d-flex align-items-center gap-2 mb-3">
                        ...
                        <h4 class="font-outfit font-bold text-white mb-0">API Request Builder</h4>
                    </div>
                    ...
                    <form id="apiRunnerForm" onsubmit="executeRequest(event)">
                        <div class="row g-3 mb-4">
                            <div class="col-md-4">
                                <label class="form-label text-slate-400 small font-bold text-uppercase">HTTP Method</label>
                                <select id="requestMethod" class="form-select text-white border-slate-700 bg-slate-900" style="background-color: #0f172a !important;">
                                    <option value="GET">GET</option>
                                    <option value="POST">POST</option>
                                </select>
                            </div>
                            <div class="col-md-8">
                                <label class="form-label text-slate-400 small font-bold text-uppercase">Endpoint Route</label>
                                <div class="input-group">
                                    <span class="input-group-text bg-slate-950 border-slate-700 text-slate-400 font-mono text-sm" style="border-right: none;">
                                        {{ url('/') }}
                                    </span>
                                    <input type="text" id="requestPath" class="form-control border-slate-700 bg-slate-900 text-white font-mono text-sm shadow-none" style="border-left: none;" placeholder="/api/v1/tasks" value="/api/v1/tasks" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                <label class="form-label text-slate-400 small font-bold text-uppercase">HTTP Request Headers (JSON)</label>
                                <textarea id="requestHeaders" rows="5" class="form-control text-white border-slate-700 bg-slate-950 font-mono text-xs" style="background-color: #020617 !important; border-color: #1e293b !important; line-height: 1.5;" placeholder="JSON headers...">{
  ...
  "x-api-key": "mgr_live_your_key_here"
                            ...
                            <div class="col-md-6">
                                <label class="form-label text-slate-400 small font-bold text-uppercase">Request Payload / Body (JSON)</label>
                                <textarea id="requestBody" rows="5" class="form-control text-white border-slate-700 bg-slate-950 font-mono text-xs" style="background-color: #020617 !important; border-color: #1e293b !important; line-height: 1.5;" placeholder="JSON payload (only for POST requests)...">{}</textarea>
                            </div>
                        </div>
                    ...
                    <div id="responseContainer" class="d-none mt-2">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <h5 class="text-slate-200 font-bold mb-0">Response Console</h5>
                            <div class="d-flex gap-2">
                                <span class="badge text-xs bg-slate-950 border border-slate-800 text-slate-300 px-2 py-1 font-mono" id="respStatus">Status: -</span>
                                <span class="badge text-xs bg-slate-950 border border-slate-800 text-slate-300 px-2 py-1 font-mono" id="respTime">Latency: -</span>
                            </div>
                        </div>
                        <div class="bg-slate-950 rounded-4 border border-slate-800 position-relative p-3">
                            <pre class="m-0 text-slate-300 font-monospace text-xs overflow-x-auto" id="respBody" style="max-height: 350px; white-space: pre-wrap; font-family: monospace;">{}</pre>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="tab-pane fade" id="sandbox" role="tabpanel" aria-labelledby="sandbox-tab">
                <div class="card glass-card p-4">
                    <div class="alert alert-warning border-0 text-white rounded-3 small p-3 mb-4" style="background-color: rgba(245, 158, 11, 0.15); border-left: 4px solid #f59e0b !important;">
                    ...
                    <h5 class="text-white font-outfit font-bold mb-3">Database Reference (Active Team IDs)</h5>
                            ...
                            <tbody>
                                @forelse($teamMembers as $member)
                                    <tr>
                                        ...
                                        <td class="text-slate-400">{{ $member->role }}</td>
    ...
    <div class="col-12 col-lg-5">
        <div class="glass-card border border-slate-800 rounded-4 shadow-lg p-4 sticky-top" style="top: 90px; max-height: calc(100vh - 120px); overflow-y: auto;">
            <!-- Language Selector tabs -->
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5 class="font-outfit font-bold text-slate-100 mb-0">API Documentation</h5>
            </div>
            ...
            <nav class="nav nav-pills gap-1 mb-4 d-flex" id="api-tabs" role="tablist" style="overflow-x: auto; white-space: nowrap; flex-wrap: nowrap; padding-bottom: 5px;">
                <button class="btn btn-outline-secondary btn-sm active py-1 px-3 text-xs text-white" id="tab-curl" onclick="switchDocLang('curl')">cURL</button>
                <button class="btn btn-outline-secondary btn-sm py-1 px-3 text-xs text-white" id="tab-php" onclick="switchDocLang('php')">PHP</button>
                <button class="btn btn-outline-secondary btn-sm py-1 px-3 text-xs text-white" id="tab-node" onclick="switchDocLang('node')">NodeJS</button>
                <button class="btn btn-outline-secondary btn-sm py-1 px-3 text-xs text-white" id="tab-python" onclick="switchDocLang('python')">Python</button>
                <button class="btn btn-outline-secondary btn-sm py-1 px-3 text-xs text-white" id="tab-java" onclick="switchDocLang('java')">Java</button>
                <button class="btn btn-outline-secondary btn-sm py-1 px-3 text-xs text-white" id="tab-ruby" onclick="switchDocLang('ruby')">Ruby</button>
            </nav>

            <!-- Interactive Route Buttons -->
            <h6 class="text-slate-400 small font-bold text-uppercase tracking-wider mb-3">API Routes Catalog</h6>
            ...
            <div id="snippetWrapper" class="mt-4 border-top border-slate-800 pt-3">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <h6 class="text-slate-200 font-bold mb-0">Code Snippet (<sup id="snippetLangLabel">cURL</sup>)</h6>
                    <button class="btn btn-xs btn-outline-secondary" onclick="copySnippet()">Copy Snippet</button>
                </div>
                <div class="bg-slate-950 rounded-4 border border-slate-800 position-relative p-3">
                    <pre class="m-0 text-slate-300 font-monospace text-xs overflow-x-auto" id="snippetPre" style="max-height: 250px; white-space: pre; font-family: monospace;"></pre>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="generateKeyModal" tabindex="-1" aria-labelledby="generateKeyModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content text-white border border-slate-800 rounded-4" style="background-color: #0b0f19;">
            ...
            <form action="{{ route('developer.keys.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label for="label" class="form-label text-slate-400 small font-bold text-uppercase">Key Description / Label</label>
